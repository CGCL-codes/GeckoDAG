...
type TxDealerHandler func (tx dagchain.GeneralTx, tc *dagchain.TxContent, isTU bool, local bool) error
...
func MakeBasicHost(listenPort int, privateKey crypto.PrivKey, fullAddrsPath string) (core.Host, error) {
...
func OpenP2PListen(pid string, host core.Host, txDealer TxDealerHandler ) {
...
func readTxFromP2P(r *bufio.Reader, txDealer TxDealerHandler) {
	...
	for {
		...
		if err := txDealer(&tx, &dagchain.TxContent{}, false, false); err!=nil {
			log.Fatal(err)
		}
	}
}
...
func ConnectP2PNode(dest *string, pid string, host core.Host, txDealer TxDealerHandler) error {
...
func SyncGT(gt dagchain.GeneralTx) error {
